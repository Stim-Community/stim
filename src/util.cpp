...
const char * const BITCOIN_CONF_FILENAME = "stim.conf";
const char * const BITCOIN_PID_FILENAME = "stimd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "stim" is a composite category enabling all Stim-related debug output
            if(ptrCategory->count(string("stim"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\StimCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\StimCore
    // Mac: ~/Library/Application Support/StimCore
    // Unix: ~/.stimcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "StimCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/StimCore";
#else
    // Unix
    return pathRet / ".stimcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty stim.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override stim.conf
